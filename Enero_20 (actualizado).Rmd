...
victimas <- read_excel("Estatal-Victimas-2015-2021_ene2021.xlsx")
library(dplyr)
library(tidyr)
library(lubridate)
...
proyecciones <- read.csv("pob_mit_proyecciones.csv", encoding="UTF8")
...
```
![](Encabezado.png)
<p style='text-align: right;'> Ficha informativa: 22 de febrero de 2021 </p>
<center>
...
<p>&nbsp;</p>
En la gráfica 3 se presentan los subtipos de delitos agrupados por bien jurídico. `r tree$Delito[1]` es el subtipo de delito que más víctimas del sexo femenino padeció en `r tolower(mes)` del presente año, con un registro de `r tree$Carpetas[1]` víctimas, en segundo lugar se encuentra, `r ifelse(tree$Carpetas[2]==tree$Carpetas[3], paste( tolower(tree$Delito[2]), "y", tolower(tree$Delito[3]),",que abrieron", tree$Carpetas[3], "cada uno"), paste0(tolower(tree$Delito[2])," ", "con"," ", tree$Carpetas[2], " ", "casos","; y en tercer lugar,"))`  `r ifelse(tree$Carpetas[3]==tree$Carpetas[4], paste0("son"," ",tolower(tree$Delito[3])," ","y"," ",tolower(tree$Delito[4]),", que abrieron"," ",tree$Carpetas[4]," ","cada uno"), paste0(tolower(tree$Delito[3])," " ,"con"," ",tree$Carpetas[3]," ", "casos"))`. 
```{r, include=FALSE}
...
```
Los presuntos delitos que experimentaron incrementos en `r tolower(mes)` fueron: `r tolower(subtipo2$label[subtipo2$Diferencia>0])[1:length(subtipo2$label[subtipo2$Diferencia>0])-1]`  `r tolower(subtipo2$label[length(subtipo2$label[subtipo2$Diferencia>0])])`. Por otro lado, los presuntos delitos que presentaron disminución fueron: `r tolower(subtipo2$label[subtipo2$Diferencia<0])[1:length(subtipo2$label[subtipo2$Diferencia<0])-1]` y `r tolower(subtipo2$label[7])`. (Tabla 1).
<p>&nbsp;</p>
...
Se anexa el link de la base de datos con la que se elaboró la ficha. [Base de datos](https://iieg.gob.mx/ns/wp-content/uploads/2021/02/victimas_Enero2021.csv)
